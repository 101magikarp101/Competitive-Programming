...
int N, M;
int a[500005], ks[500005];
pii b[500005];
int ans[500005];

map<int, vt<pii>> m; // 0->ans, 1->dx, 2->query
...
int main() {
    ...
    rep(i,0,N) {
        ...
        int l = *it;
        int iv = inv(b[i].x);
        int cnt = mul(r-b[i].y, b[i].y-l);
        int cnt1 = mul(N-b[i].y, b[i].y+1);
        m[1].pb({1, mul(iv, cnt)});
        m[b[i].x].pb({1, sub(cnt, mul(iv, cnt))});
        cur = ad(cur, mul(iv, cnt1));
        s.insert(b[i].y);
    }
    rep(i,0,M) {
        m[ks[i]].pb({2, i});
