...
int main() {
    ...
    dist[1] = 0;
    ways[1] = 1;
    priority_queue<state, vt<state>, greater<state>> pq; // min-heap by dist
    // priority_queue<state> pq; // same thing
    pq.push({1,0});
    ...
    while (!pq.empty()) {
        ...
        for (edge e : adj[s.node]) {
            ...
            if (ndist < dist[e.v]) {
                dist[e.v] = ndist;
                ways[e.v] = ways[s.node];
                minf[e.v] = minf[s.node] + 1;
                maxf[e.v] = maxf[s.node] + 1;
                pq.push({e.v, ndist});
            } else if (ndist == dist[e.v]) {
                ways[e.v] = ad(ways[e.v], ways[s.node]);
                ckmin(minf[e.v], minf[s.node] + 1);
                ckmax(maxf[e.v], maxf[s.node] + 1);
            }
        }
    }
