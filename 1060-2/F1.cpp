...
int solve(int x) {
    ...
    rep(i,0,N) {
        ...
        if (a[i] <= x) {
            int dist = a[i]-prevd-1 - remd;
            // dist -= remd;
            ans = mul(ans, nck(unused, dist));
            prevd = a[i];
            remd = 0;
            remu += unused - dist;
        } else {
            int dist = a[i]-prevu-1 - remu;
            // dist -= remu;
            ans = mul(ans, nck(unused, dist));
            prevu = a[i];
            remu = 0;
    ...
    // cout << "ans before final=" << ans << " remd=" << remd << " unused=" << unused << endl;
    ans = mul(ans, nck(unused, x-prevd - remd));
