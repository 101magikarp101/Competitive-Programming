...
public:
    ...
    SegTree(vt<T> &a) {
        ...
        rrep(i,len-1,1) tr[i] = merge(tr[i*2], tr[i*2+1]);
    }
    void ckmin(int i, T v) {
        i += len;
        tr[i] = merge(tr[i], {v, i-len});
        for (i >>= 1; i > 0; i >>= 1) {
...
int main() {
    ...
    for (int x = -1; x <= 1; x += 2) {
        ...
        vt<pii> res = solve(b);
        rep(i,0,N) ans[i] = merge(ans[i], res[i]);
        rep(i,0,N) {
            swap(b[i].x, b[i].y);
        }
        res = solve(b);
