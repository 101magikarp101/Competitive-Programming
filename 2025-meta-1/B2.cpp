...
int nck(ll n, ll k) {
    int ans = 1;
    rep(i,0,k) {
        ans = mul(ans, mul((n-i)%MOD2, invs[i+1]));
    }
    return ans;
}

int solve(vt<pll> &v, int i, ll x, int y) {
    if (i == sz(v)) {
        if (x <= A) return y;
        else return 0;
    }
    int ans = 0;
    rep(j,0,v[i].y+1) {
        ans = ad(ans, solve(v, i+1, x*(ll)pow(v[i].x,j), mul(y, mul(nck(N+j-1, j), nck(N+(v[i].y-j-1), v[i].y-j)))));
    }
    return ans;
}
