...
int main() {
    ...
    rep(i,2,N+1) {
        pre[i] = pre[i-1] + dist(i, i-1);
    }
    int ans = pre[N];
    rep(i,1,N+1) {
        rep(j,i+1,N+1) {
            if (j == i+1) {
                int res = pre[i-1] + dist(i-1, j) + dist(i, j) + dist(i, j+1) + (j < N ? pre[N] - pre[j+1] : 0);
                // cout << "i=" << i << ", j=" << j << ", res=" << res << endl;
                ckmin(ans, res);
            } else {
                int res = pre[i-1] + dist(i-1, j) + dist(i+1, j) + (pre[j-1] - pre[i+1]) + dist(i, j-1) + dist(i, j+1) + (j < N ? pre[N] - pre[j+1] : 0);
                // cout << "i=" << i << ", j=" << j << ", res=" << res << endl;
                ckmin(ans, res);
            }
        }
