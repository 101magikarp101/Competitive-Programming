...
int solve2(int u, int v) {
    if (dp2[u][v] != -1) return dp2[u][v];
    int &res = dp2[u][v];
    res = solve(u, v);
    each(x,adj[u]) {
        res = ad(res, solve2(x,v));
    }
    each(y,adj[v]) {
        res = ad(res, solve2(u,y));
    }
    each(x,adj[u]) {
        each(y,adj[v]) {
            res = sub(res, solve2(x,y));
        }
    }
    return res;
}
...
int main() {
    ...
    while (T--) {
        ...
        cin >> s;
        s = " " + s;
        rep(i,1,N+1) adj[i].clear(), par[i] = -1;
        rep(i,1,N+1) rep(j,1,N+1) dp[i][j] = -1, dp2[i][j] = -1;
        rep(i,0,N-1) {
            int u, v; cin >> u >> v;
            adj[u].pb(v);
            adj[v].pb(u);
