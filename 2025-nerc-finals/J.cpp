...
double solve(int i, int k) { // rounds used, wins
    if (i == K) {
        return 1;
    }
    if (dp[i][k] != -1.0) return dp[i][k];
    double res1 = solve(i+1, k+1);
    double res2 = solve(i+1, k);
    double num = 0;
    double den = 0;
    rep(j,0,M+1) {
        double r = (double)j/M;
    ...
    double prob = num/den;
    double mult = res1*prob*2;
    double safe = prob*res1 + (1-prob)*res2;
    // cout << "i=" << i << " k=" << k << " mult=" << mult << " prob=" << prob << endl;
    // cout << "i=" << i << " k=" << k << " mult=" << mult << " prob=" << prob << endl;
    // cout << "res1=" << res1 << " res2=" << res2 << endl;
    dp[i][k] = max(mult, safe);
