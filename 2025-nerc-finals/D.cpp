...
int main() {
    ...
    int ans = 0;
    while (!pq.empty()) {
        section s = pq.top(); pq.pop();
        // cout << "Processing section l=" << s.l << " r=" << s.r << endl;
        ckmax(ans, s.r - *ls.begin());
        if (s.q->empty()) break;
        int v = s.q->front(); s.q->pop();
        ls.erase(ls.find(s.l));
        s.l += v;
        ls.insert(s.l);
        s.r += v;
        pq.push(s);
