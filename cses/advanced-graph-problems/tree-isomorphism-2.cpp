...
void dfs(int u, int p, vvi &adj, vi &dep, int d, vi &prev) {
    dep[u] = d;
    prev[u] = p;
    each(v, adj[u]) {
        if (dep[v] == -1) {
            dfs(v, u, adj, dep, d+1, prev);
        }
    }
}
...
int solve(vvi &adj) {
    ...
    int n = sz(path);
    if (n%2==1) {
        return ha(path[n/2], -1, adj);
    } else {
        adj.pb(vi());
        int m = sz(adj);
        adj[m-1].pb(path[n/2-1]);
        adj[m-1].pb(path[n/2]);
        return ha(m-1, -1, adj);
    }
}
