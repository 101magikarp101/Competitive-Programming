...
int main() {
    ...
    while (Q--) {
        ...
        if (t == 0) {
            ...
            int pre = bit.sum(x); // < x
            int total = bit.sum(N);
            if (pre == total) cout << -1 << endl;
            else cout << bit.walk(pre + 1) - 1 << endl;
        } else if (t == 4) {
            int upto = bit.sum(x + 1); // <= x
            if (upto == 0) cout << -1 << endl;
            else cout << bit.walk(upto) - 1 << endl;
        }
    }
