...
int main() {
    ...
    while (Q--) {
        ...
        ll opt = B/(2*A);
        // cout << "optimal price: " << opt << endl;
        if (opt >= M) ckmax(ans, f(A, B, C, M));
        else if (opt < 0) ckmax(ans, f(A, B, C, 0));
        else {
            if (0 <= opt && opt <= M) ckmax(ans, f(A, B, C, opt));
            if (0 <= opt+1 && opt+1 <= M) ckmax(ans, f(A, B, C, opt+1));
        }
        cout << ans << endl;
    }
